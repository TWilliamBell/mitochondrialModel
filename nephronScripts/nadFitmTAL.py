...
nadhIC = pc.ics[pc.pcIS.iNADH_x]

def main(): ## Runs differential equation for time span and outputs results to
    ## a csv file and a feather file.
    for i in range(10):
        pc.pcPC.NADtot = nadtotnorm/(i+20)
        pc.ics[pc.pcIS.iNADH_x] = \
            min(pc.pcPC.NADtot/2.0, nadhIC)
        J_AtC = 1.318e-3
        ...
        results = pd.DataFrame(results,
                            columns = ["t", "H_x", "dPsi", "ATP_x", "ADP_x",
                                      ...
                                      "PCr_c", "AMP_c"])
        results.to_csv("../results/resultsmTALNAD"+str(i)+".csv")
