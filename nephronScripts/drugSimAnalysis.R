...
pdf("./dataVis/ifosfamideATP.pdf", width = 9)
par(cex.lab = 1.5, cex.axis = 2.5)
plot(dcaATP, ylim = c(0.0001, max(dcaATP)), xlim = c(0, 5),
     xaxt = "n", xlab = "Conditions", cex = 0.1, ylab = "[ATP]_c (M)")
rect(1:4-0.5, rep(0, 4), 1:4+0.5, dcaATP, col = "springgreen")
axis(side = 1, at = 1:4, labels = c("DCA+/Ifos+", "DCA+/Ifos-", 
                "DCA-/Ifos+", "DCA-/Ifos-"))
dev.off()
