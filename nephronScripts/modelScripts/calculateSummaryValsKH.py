...
def main():
    ...
    khUp = pd.read_csv("../results/resultsKH100.csv").tail(1).to_numpy()[0]
    khUp = np.delete(khUp, [0, 1])

    reg = pd.read_csv("../results/resultsKHbase.csv").tail(1).to_numpy()[0]
    reg = np.delete(reg, [0, 1])

    ## Data loading and cleaning, based on measureVars.py simulations
    s2 = pd.read_csv("../results/resultsState2KH.csv").tail(1).to_numpy()[0]
    s2 = np.delete(s2, [0, 1])
    ...
    s3 = pd.read_csv("../results/resultsState3KH.csv")
    ...
    lr = pd.read_csv("../results/resultsLeakRespKH.csv")
    ...
    po = pd.read_csv("../results/resultsPOKH.csv")
