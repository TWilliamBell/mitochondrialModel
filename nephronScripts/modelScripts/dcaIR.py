...
def f(t, y): ## Differential equations, with optional arguments specified
    ...
    return equations.conservationEqs1(y, J_AtC = J_AtC,
                              ExpType = ExpType, DCA = DCA,
                              StateType = StateType)

def g(t, y): ## Differential equations, with optional arguments specified
    print(t)
    yprime = equations.conservationEqs1(y, J_AtC = J_AtC,
                              ExpType = ExpType,
                              StateType = StateType)
    #yprime[pc.pcIS.iPYR_c] = 0.01*(normPYR-y[pc.pcIS.iPYR_c])
    return yprime
...
def main(): ## Runs differential equation for time span and outputs results to
    ...
    if count == 0:
        ...
        final = np.delete(final.tail(1).to_numpy()[0], [0, 1])
